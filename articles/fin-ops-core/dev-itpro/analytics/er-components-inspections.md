...
<tr>
<td><a href='#i15'>ไม่มีการแม็ปเริ่มต้น</a></td>
<td>ความสมบูรณ์ของข้อมูล</td>
<td>ผิดพลาด</td>
<td>
<p>มีการแม็ปแบบแผนที่มากกว่าหนึ่งแบบเกิดขึ้นสำหรับการกำหนดค่าคอนฟิก &lt;ชื่อแบบจำลอง (ตัวบอกราก)&gt; แบบจำลองข้อมูลในการกำหนดค่าคอนฟิก &lt;ที่เรียกแยกกันโดยมีเครื่องหมายจุลภาคคั่น&gt; ตั้งค่าการกําหนดค่าคอนฟิกค่าใดค่าหนึ่งเป็นค่าเริ่มต้น</p>
...
3. ในตัวออกแบบการแม็ปแบบจำลอง ในบานหน้าต่าง **ที่มาของข้อมูล** ให้เพิ่มที่มาของข้อมูลของชนิด **ฟิลด์จากการคำนวณ**
...
7. เลือก **ตรวจสอบความถูกต้อง** เพื่อตรวจสอบส่วนประกอบของการแม็ปแบบจำลองที่แก้ไขได้บนหน้า **ตัวออกแบบการแม็ปแบบจำลอง**
    ...
    ![การตรวจสอบความถูกต้องส่วนประกอบของการแม็ปแบบจำลองที่แก้ไขได้บนหน้าตัวออกแบบการแม็ปแบบจำลอง](./media/er-components-inspections-01.gif)
...
4. ตั้งชื่อแหล่งข้อมูล **Y** ใหม่และตั้งค่าคอนฟิกเพื่อให้มีนิพจน์ `INTVALUE(100)`
5. ผูก **X** กับ **Y**
6. ในตัวออกแบบการแม็ปแบบจำลอง ในบานหน้าต่าง **แหล่งข้อมูล** ให้ลบแหล่งข้อมูล **Y**
7. เลือก **ตรวจสอบความถูกต้อง** เพื่อตรวจสอบส่วนประกอบของการแม็ปแบบจำลองที่แก้ไขได้บนหน้า **ตัวออกแบบการแม็ปแบบจำลอง**
...
ในตัวออกแบบการแม็ปแบบจำลอง ในบานหน้าต่าง **แหล่งข้อมูล** ให้เพิ่มแหล่งข้อมูล **Y** อีกครั้ง
...
8. ตั้งชื่อแหล่งข้อมูล **$AccNumber** ใหม่และตั้งค่าคอนฟิกเพื่อให้มีนิพจน์ `TRIM(Trans.AccountNum)`
    ...
    ![ตั้งค่าคอนฟิกแหล่งข้อมูลในหน้าตัวออกแบบการแม็ปแบบจำลอง](./media/er-components-inspections-05a.png)
...
![นิพจน์ที่อัปเดตของแหล่งข้อมูลในหน้าตัวออกแบบการแม็ปแบบจำลอง](./media/er-components-inspections-06d.png)
...
![การเลือก Fix ที่เลือกไว้บนหน้าตัวออกแบบการแม็ปแบบจำลอง](./media/er-components-inspections-08c.png)
...
5. ปรับเปลี่ยนสินค้า **ผู้จัดจำหน่าย** ในลักษณะต่อไปนี้:
    ...
    ![การเพิ่มฟิลด์ที่ซ้อนกันบนหน้ารูปแบบข้อมูล](./media/er-components-inspections-09a.png)

6. ในตัวออกแบบการแม็ปแบบจำลอง ในบานหน้าต่าง **แหล่งข้อมูล** ให้เพิ่มแหล่งข้อมูลของชนิด **Dynamics 365 for Operations \\ ตารางบันทึก**
7. ตั้งชื่อแหล่งข้อมูล **ผู้จัดจำหน่าย** ใหม่ ในฟิลด์ **ตาราง** ให้เลือก **VendTable** เพื่อระบุว่าแหล่งข้อมูลนี้จะร้องขอตาราง VendTable
...
12. ผูกรายการแบบจำลองข้อมูลเพื่อตั้งค่าคอนฟิกแหล่งข้อมูลในลักษณะต่อไปนี้:
    ...
    ![การผูกรายการแบบจำลองข้อมูลในหน้าตัวออกแบบการแม็ปแบบจำลอง](./media/er-components-inspections-09b.png)
...
ภาพประกอบต่อไปนี้แสดงข้อผิดพลาดรันไทม์ที่เกิดขึ้นถ้าคุณละเว้นคำเตือน เลือก **รัน** เพื่อรันรูปแบบ และเลือกหมายเลขบัญชีของผู้จัดจำหน่ายที่ไม่มีอยู่ เนื่องจากไม่มีผู้จัดจำหน่ายที่ร้องขออยู่ รายการ `model.Vendor` จะว่างเปล่า (นั่นคือ จะไม่มีเรกคอร์ด)
...
คุณสามารถผูกองค์ประกอบรูปแบบ **ใบแจ้งยอด\\ฝ่าย\\ชื่่อ** เข้ากับกับรายการแหล่งข้อมูล `model.Vendor` เมื่อรันไทม์ การผูกข้อมูลนี้เรียกแหล่งข้อมูล `model.Vendor` เป็นอันดับแรก เมื่อ `model.Vendor` ส่งคืนรายการเรกคอร์ดที่ว่างเปล่า จะไม่มีการรันองค์ประกอบรูปแบบซ้อนกัน ดังนั้น จึงไม่มีคำเตือนการตรวจสอบความถูกต้องสำหรับการตั้งค่าคอนฟิกรูปแบบนี้
...
12. ผูกรายการแบบจำลองข้อมูลเพื่อตั้งค่าคอนฟิกแหล่งข้อมูลในลักษณะต่อไปนี้:
    ...
    ![รายการรูปแบบข้อมูลที่ผูกกับแหล่งข้อมูลที่ถูกกำหนดค่าคอนฟิกและรายการรูปแบบข้อมูลที่ยังไม่ได้ผูกไว้บนหน้าตัวออกแบบการแม็ปแบบจำลอง](./media/er-components-inspections-11b.png)
...
## <a name="not-synced-with-a-word-template-format"></a><a id="i14"></a>ไม่ได้ซิงค์กับรูปแบบเท็มเพลต Word
...
ด้วยวิธีนี้ คุณบ่งชี้ว่าองค์ประกอบเพิ่มเติมจะเติมแม่แบบที่เลือกเมื่อรันไทม์ เนื่องจากเทมเพลต Word ที่เพิ่มขึ้นมีการออกแบบภายนอก รูปแบบ ER ที่สามารถแก้ไขได้อาจมีการอ้างอิงถึงตัวควบคุมเนื้อหา Word ที่ขาดหายไปจากเทมเพลตที่เพิ่มขึ้น ตัวออกแบบรูปแบบ ER จะเตือนคุณเกี่ยวกับความไม่สอดคล้องกันระหว่างคุณสมบัติของส่วนประกอบรูปแบบ ER ซึ่งอ้างอิงถึงตัวควบคุมเนื้อหาที่ไม่ได้รวมอยู่ในแม่แบบ Word ที่เพิ่มขึ้น
...
แก้ไขการใช้เท็มเพลต Word ด้วย [การเพิ่ม](er-design-configuration-word-suppress-controls.md#tag-control) แท็กที่ต้องใช้ในการควบคุมเนื้อหา Word ที่เกี่ยวข้อง

## <a name="no-default-mapping"></a><a id="i15"></a>ไม่มีการแม็ปเริ่มต้น

เมื่อได้ดำเนินการตรวจสอบ [การผูกข้อมูลที่ขาดหายไป](#i11) แล้ว การผูกข้อมูลรูปแบบที่ตรวจสอบจะประเมินเทียบกับการผูกมัดของส่วนประกอบการแม็ปแบบโมเดลที่เกี่ยวข้อง เนื่องจากคุณสามารถนำเข้าการกำหนดค่าการแม็ปโมเดล ER [จำนวนมาก](./tasks/er-manage-model-mapping-configurations-july-2017.md) ไปยังอินสแตนซ์ Finance ของคุณได้ และค่าคอนฟิกแต่ละค่าอาจมีส่วนประกอบการแม็พโมเดลที่เกี่ยวข้อง ค่าคอนฟิกหนึ่งจะต้องถูกเลือกเป็นค่าคอนฟิกเริ่มต้น มิฉะนั้น เมื่อคุณพยายามรัน แก้ไข หรือตรวจสอบความถูกต้องของรูปแบบ ER ที่ตรวจสอบแล้ว จะเกิดข้อยกเว้นขึ้น และคุณจะได้รับข้อความต่อไปนี้ "มีการแม็ปแบบลองมากกว่าหนึ่งรายการสำหรับ \<model name (root descriptor)\> แบบจำลองข้อมูลในค่าคอนฟิก \<configuration names separated by comma\> ตั้งค่าการกําหนดค่าคอนฟิกค่าใดค่าหนึ่งเป็นค่าเริ่มต้น
...
[จัดการการแม็ปที่ได้รับมาหลายรายการสำหรับรากของแบบจำลองเดียว](er-multiple-model-mappings.md)
