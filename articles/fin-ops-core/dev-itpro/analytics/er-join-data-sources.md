...
ในแหล่งข้อมูล **การรวม** ที่กำหนดค่าไว้ เมื่อแหล่งข้อมูลทั้งหมดเป็นชนิด **เรกคอร์ดตาราง** การดำเนินการของแหล่งข้อมูล Join สามารถ [ดำเนินการที่ระดับฐานข้อมูลได้](#analyze) โดยใช้คำสั่ง SQL เดี่ยว การทำเช่นนี้จะช่วยลดจำนวนของการเรียกฐานข้อมูล ซึ่งจะช่วยปรับปรุงประสิทธิภาพของการแม็ปแบบจำลอง มิฉะนั้น การดำเนินการของ **ข้อมูลการรวม** จะดำเนินการในหน่วยความจำ
...
นอกจากนี้ คุณจำเป็นต้องดาวน์โหลดจาก [ศูนย์ดาวน์โหลด Microsoft](https://go.microsoft.com/fwlink/?linkid=000000) และบันทึกไฟล์ตัวอย่างการกำหนดค่า ER ไว้ภายในเครื่อง

| **คำอธิบายเนื้อหา**  | **ชื่อไฟล์**   |
|--------------------------|-----------------|
| ไฟล์ตัวอย่างการกำหนดค่า **แบบจำลองข้อมูล ER** ซึ่งใช้เป็นแหล่งข้อมูลสำหรับตัวอย่าง| [แบบจำลองเพื่อเรียนรู้แหล่งข้อมูล JOIN.รุ่น.1.1.xml](https://mbs.microsoft.com/customersource/Global/AX/downloads/hot-fixes/365optelecrepeg) |
| ไฟล์ตัวอย่างการกำหนดค่า **การแม็ปแบบจำลอง ER** ซึ่งจะนำไปใช้งานกับแบบจำลองข้อมูล ER สำหรับตัวอย่าง | [การแม็ปเพื่อเรียนรู้แหล่งข้อมูล JOIN.รุ่น.1.1.xml](https://mbs.microsoft.com/customersource/Global/AX/downloads/hot-fixes/365optelecrepeg) |
| ไฟล์ตัวอย่างการกำหนดค่า **รูปแบบ ER** ไฟล์นี้จะอธิบายถึงข้อมูลที่จะนำไปเติมเป็นส่วนประกอบของรูปแบบ ER สำหรับตัวอย่าง | [รูปแบบเพื่อเรียนรู้แหล่งข้อมูล JOIN.รุ่น.1.1.xml](https://mbs.microsoft.com/customersource/Global/AX/downloads/hot-fixes/365optelecrepeg) |
...
6.  สังเกตรายการของการกำหนดค่า ER ในแผนภูมิและรายละเอียดรุ่นบนแท็บด่วนของ **รุ่น** ซึ่งจะใช้เป็นแหล่งของข้อมูลสำหรับรายงานตัวอย่างของคุณ
...
5.  ในแผนภูมิการกำหนดค่า ขยายแบบจำลองข้อมูล **Set1** และ **Set1.Details** ของรายการ:
    ...
    1. การผูก **Details: Record list = Versions** บ่งชี้ถึงรายการ **Set1.Details** ถูกผูกกับแหล่งข้อมูล **รุ่น** ส่งคืนเรกคอร์ดของตาราง **ERSolutionVersionTable** แต่ละเรกคอร์ดของตารางนี้แสดงถึงรุ่นเดียวของการกำหนดค่า ER เนื้อหาของตารางนี้แสดงอยู่ในแท็บด่วน **รุ่น** ในเพจ **การกำหนดค่า**
...
### <a name="review"></a>ตรวจทานการแม็ปแบบจำลอง ER (ส่วนที่ 2)
...
1.  ในแผนภูมิการกำหนดค่า ขยายแบบจำลองข้อมูล **Set2** และ **Set2.Details** ของรายการ: โปรดทราบว่าการผูก **Details: Record list = Details** บ่งชี้ว่ารายการ **Set2.Details** ถูกผูกกับแหล่งข้อมูล **รายละเอียด** ตามที่กำหนดค่าเป็นแหล่งข้อมูลของชนิด **การรวม**
...
5.  เลือก **แสดงรายละเอียด**
    ...
    โปรดทราบว่าการรวมเรกคอร์ดของแหล่งข้อมูลแสดงถึงตารางแอปพลิเคชัน คุณสามารถระบุเงื่อนไขการรวมโดยการใช้คู่ของฟิลด์นอกจากที่ใช้อธิบายความสัมพันธ์ AOT ระหว่างตารางที่มีอยู่ ชนิดของการรวมนี้สามารถกดหนดค่าเพื่อดำเนินการที่ระดับฐานข้อมูลได้
...
8.  ในแผนภูมิการกำหนดค่า ขยายแบบจำลองข้อมูล **Set2.Summary** ของรายการ:
    ...
    - โปรดทราบว่าตัวเลือกสถานที่ **การดำเนินการ** ถูกกำหนดค่าเป็น **การสอบถาม** หมายความว่าแหล่งข้อมูล **GroupBy** นี้จะถูกดำเนินการเมื่อรันไทม์เป็นการเรียก SQL โดยตรงที่ระดับฐานข้อมูล การทำเช่นนี้เป็นไปได้เนื่องจากแหล่งข้อมูลฐาน **รายละเอียด** ของชนิด **การรวม** ถูกกำหนดค่าเป็นดำเนินการที่ระดับฐานข้อมูล
...
### <a name="executeERformat"></a> ดำเนินการรูปแบบ ER
...
8.  เลือก **ขยาย/ยุบ**
    ...
    โปรดทราบว่ารูปแบบนี้ได้รับการออกแบบมาเพื่อเติมข้อมูลไฟล์ข้อความที่สร้างขึ้นด้วยรายการใหม่สำหรับทุกๆ รุ่นของการกำหนดค่า ER (ลำดับ **รุ่น**) แต่ละรายการที่ถูกสร้างขึ้นจะประกอบด้วยชื่อของผู้ให้บริการการกำหนดค่าที่เป็นเจ้าของการกำหนดค่าปัจจุบัน ชื่อของการกำหนดค่า และรุ่นของการกำหนดค่าถูกคั่นด้วยเครื่องหมายอัฒภาค รายการสุดท้ายที่สร้างขึ้นจะประกอบด้วยจำนวนของรุ่นของการกำหนดค่า ER ที่พบ (ลำดับ **สรุป**)
...
4.  เลือก **ตกลง**
    ...
    โปรดทราบว่าสถิติการเรียกใช้จะช่วยแจ้งให้คุณทราบถึงการเรียกใช้ไปยังตารางแอปพลิเคชันที่ซ้ำกัน
...
#### <a name="analyze"></a> วิเคราะห์การติดตามการเรียกใช้รูปแบบ ER
...
4.  เลือก **ตกลง**
    ...
    โปรดสังเกตว่าสถิติการดำเนินการจะแจ้งให้คุณทราบถึงสิ่งต่อไปนี้:
